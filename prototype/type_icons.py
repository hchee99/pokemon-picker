...
TYPE_RGB = {
    ...
    "풀": (94,155,63), "얼음": (121,208,242), "격투": (236,149,79), "독": (136,77,196),
    "땅": (135,83,47), "비행": (138,177,229), "에스퍼": (216,85,125), "벌레": (151,161,66),
    "바위": (188,184,137), "고스트": (103,64,112), "드래곤": (85,102,210), "악": (79,71,71),
...
VARIANTS = [("얼음", (150,220,248)), ("얼음", (170,228,250)),
            ("전기", (234,200,88)), ("전기", (238,208,102)),
            ("전기", (250,192,0)), ("전기", (243,191,40)),   # 실측: 밝은 경기장 쨍한 노랑
            ("물", (40,128,240)),                            # 실측: 물 아이콘
            ("격투", (235,136,52)), ("격투", (240,118,42)), ("격투", (248,100,28))]
...
REJECT_RGB = [(99,23,47),(110,36,44),(101,36,58),(98,34,76),(98,27,73),(120,40,66),(70,18,38),
              ...
              (56,106,230)]  # 파란 코트 바닥 (아이콘 띠 가장자리로 비침 — 물 오인 방지)
...
def find_panels(shot):
    ...
    on = rows > rows.max()*0.18   # 어두운 포켓몬이 패널을 많이 덮어도 검출되게
    bands = []; i = 0
    while i < H:
        if on[i]:
            j = i
